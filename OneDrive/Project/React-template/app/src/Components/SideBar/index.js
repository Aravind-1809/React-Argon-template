...
const data1 = [
    {icon: <FaFile className='icon3'/>, name:"Pages", subitem:[
        {name:"Profile", sub:[
    ...
    {icon: <BiBarChart className='icon3' />, name:"Applications", subitem:[
        {name:"Kanban"},
        {name:"Wizard"},
        {name:"Data Tables"},
        {name:"Calendar"},
    ]},
    {icon: <IoCart className='icon3'/>, name:"Ecommerce", subitem:[
        {name:"Products", sub:[
            {name:"New Product"},
            {name:"Edit Product"},
            {name:"Product Page"},
    ...
    {icon: <IoMdPerson className='icon3'/>, name:"Authentication", subitem:[
        {name:"Sign In", sub:[
            {name:"Basic"},
            {name:"Cover"},
            {name:"Illustration"},
...
const SideBar = () => {
    ...
    const handleMouseEnter = () => {
        let width = $('.side-main-container').width();
        console.log(width)
        if (width < 100){
        $(".side-main-container").attr('id', 'hover')
    ...
    const handleMouseLeave = () => {
        let id = $('.side-main-container').attr('id');
        console.log(id)
        if (id === "hover"){
        $(".side-main-container").toggleClass("cont-width")
        $(".side-container").toggleClass("cont-marg")
        ...
        $(".card-carry-container").toggleClass("increase-width")
        $(".get-started-container").toggleClass("increase-width")
        $(".side-main-container").removeAttr('id')
        $("#dot").toggleClass("hide-brand")
        $("#LandingPage").toggleClass("hide-brand")
        $("#Default").toggleClass("hide-brand")
        $("#Automotive").toggleClass("hide-brand")
        ...
        <div className='side-container'>
            ...
            <div className='dashboard-container' onClick={hide}>
                <div className='inside-container'>
                    ...
                    <span id="dash" className='dash'>Dashboard</span>
                </div>
                <svg id='arrow1' className='arrow' viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path fill="currentColor" d="M16.59 8.59L12 13.17 7.41 8.59 6 10l6 6 6-6z"></path></svg>
            </div>
            <ul id="board" className='dashboard-list-1'>
            {data.map(each => {
                return <Option key={each.name} item={each}/>
            })}
            </ul>
            <span id="pages" className='page-title'>PAGES</span>
            <ul className='dashboard-list'>
                {data1.map(each => {
                    return <SubPage key={each.name} item={each} />
                })}
